...
data-showsidebar="#encodeForHTMLAttribute(
	lcase(
		yesNoFormat( prc.oCurrentAuthor.getPreference( "sidebarState", true ) )
	)
)#"
...
<div class="flex">
	...
	>#cbAdminComponent( "ui/Icon", { name: "Bars3" } )#</button>
<!--Branding-->
<div class="brand">
	<a
...
value="#event.buildLink( prc.xehSearchGlobal )#"
	...
	<cfif prc.oCurrentAuthor.hasPermission(
		"PAGES_ADMIN,PAGES_EDITOR,ENTRIES_ADMIN,ENTRIES_EDITOR,AUTHOR_ADMIN,MEDIAMANAGER_ADMIN"
	)>
...
<a data-keybinding="Control+Shift+p" href="#event.buildLink( prc.xehPagesEditor )#" title="Control+Shift+p">
		...
		<cfif prc.oCurrentSite.getIsBlogEnabled() && prc.oCurrentAuthor.hasPermission( "ENTRIES_ADMIN,ENTRIES_EDITOR" )>
			<li>
...
<a data-keybinding="Control+Shift+v" href="#event.buildLink( prc.xehMenuManager )#" title="Control+Shift+v">
	...
	#prc.adminMenuService.generateUtilsMenu()##prc.adminMenuService.generateProfileMenu()#<!--- Notifications :
			...
			<main class="main-content-wrapper" id="main-content-wrapper">
				<section id="main-content">
					...
					#view()##announce( "cbadmin_afterContent" )#
				</section>
			</main>
