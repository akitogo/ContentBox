...
<div class="controls">
	<small>Choose the default two factor provider that will be used for authentication by all ContentBox users. Please note that each provider could have different setup options. Ultimately, refer to the provider documentation.</small><br/>
          <small class="text-danger"><strong>Note:</strong> Changing the default two factor provider will unenroll all currently enrolled users!</small><br />#html.select(
...
<small>By default a logout takes you to the login screen, you can choose your own sign out URL below.</small><br/>#html.URLField(
...
<small>Custom text to show in the login screen, HTML/Markdown enabled.</small><br/>#html.textarea(
