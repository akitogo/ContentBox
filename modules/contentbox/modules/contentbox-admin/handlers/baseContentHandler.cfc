...
component extends="baseHandler" {
    ...
    function index(event, rc, prc) {
        ...
        // get all categories
        prc.categories = variables.categoryService.list(criteria = {'site': prc.oCurrentSite}, sortOrder = 'category');
    ...
    function bulkStatus(event, rc, prc, required relocateTo) {
        ...
        if (len(rc.contentID)) {
            ...
            // announce event
            announce('cbadmin_on#variables.entity#StatusUpdate', {contentID: rc.contentID, status: rc.contentStatus});
            // Message
    ...
    function save(
        ...
        oContent.addNewContentVersion(content = rc.content, changelog = rc.changelog, author = prc.oCurrentAuthor);
        ...
        // announce event
        announce('cbadmin_pre#variables.Entity#Save', {content: oContent, isNew: isNew, originalSlug: originalSlug});

        // save content
        variables.ormService.save(oContent, originalSlug != oContent.getSlug() ? originalSlug : '');

        // announce event
        announce('cbadmin_post#variables.entity#Save', {content: oContent, isNew: isNew, originalSlug: originalSlug});
