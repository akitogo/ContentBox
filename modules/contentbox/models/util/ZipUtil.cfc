			...
			try {
				...
				for ( i = 1; i LTE arrayLen( localfiles ); i = i + 1 ) {
					if ( fileExists( localfiles[ i ] ) ) {
						...
						} else // Remove directory from path
						if ( arguments.savePaths EQ "no" ) {
	...
	>
		...
		<cfscript>
			...
			for ( i = 1; i LTE dir.recordcount; i = i + 1 ) {
				...
				} else /* Get files from sub directorys and add them to the array */
				if ( dir.type[ i ] EQ "dir" && arguments.recurse EQ "yes" ) {
					subdir = filesList(
						path,
						arguments.filter,
						arguments.recurse
